...
export const routerDataHanding = apiRouterData => {
  ...
  const parentIds = ["page1", "page2"]; // 顶部菜单的id列表

  // 外层节点
  apiRouterData.forEach(route => {
    if (route.parentId === "rootMenu") {
      asyncRouterMap.push({
    ...
    } else if (parentIds.some(id => id === route.parentId)) {
      if (route.path === "") {
        // 有子节点的父级路由（模块，非页面）
        asyncRouterMap.push({
          path: route.url === "/" ? route.url : "/" + route.url,
  ...
  const handleRecurrence = recurrenceData => {
    recurrenceData.forEach(data => {
      apiRouterData.forEach(route => {
        if (
          data.meta.id === route.parentId &&
          parentIds.every(id => id !== route.parentId)
        ) {
          data.children.push({
            path: route.url,
            name: route.name,
