        ...
        <Form :model="standardForm"
          ...
          <FormItem label="所属SOP"
            ...
            <div v-if="tabList.length>0">
              <Select v-model="standardForm.sopId"
                      placeholder="请选择">
                <Option v-for="(item,i) in sopSelectList"
...
import {
  ...
  getSopByQcIndex // 获取sop下拉框 - select选择
} from "@/api/process";
  ...
  methods: {
    ...
    async tabSelect(name) {
      ...
      this.sopSelectList = !this.isMock
        ? (await getSopByQcIndex(this.tabSelected)).data.data || []
    ...
    async initCascader() {
      ...
      this.mNumberList = !this.isMock
        ...
        : mNumberList[this.tabSelected] || [];
      // isMock时的数据处理
      this.isMock && this.refreshCasData();
      // 2.默认选择第一项 -> table刷新
      if (this.mNumberList.length !== 0) {
        this.mNumberSelect = [
    ...
    async getData() {
      if (!this.isMock) {
        // 接口数据
        this.tableLoading = true;
        this.tableData = (await findBySopTag(this.mNumberSelect[1])).data.data;
        this.tableLoading = false;
        this.buttonLoading = false;
      } else {
        // mock数据
    ...
    keydown(event) {
      this.tagKeyCodeList.indexOf(event.keyCode) === -1 &&
        event.preventDefault();
    },
          ...
          if (
            ...
            if (!this.isMock) {
              ...
              resultCallback(result.data.status, "新增成功！", async () => {
                // 1.重新获取电机型号list
                this.mNumberList = (
                  await findSopTagsByQcIndex(this.tabSelected)
                ).data.data;
                // 2.被选择的选项 -> 即新增的标准
          ...
          if (!this.isMock) {
            ...
            this.mNumberList.forEach(item => {
              if (item.value === this.mNumberSelect[0]) {
                item.children.forEach((standard, i) => {
